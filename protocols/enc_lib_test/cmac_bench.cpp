...
int main(int argc, char **argv) {
  ...
  ::memcpy((ptr.get() + PacketSsl::IvSize), aad.get(), msg_sz);
  print_buffer_bytes(ptr.get(), PacketSsl::get_buffer_size(msg_sz));
  /*
  std::cout << "ptr=";
  ...
  */
  ...
  uint8_t unused;
  bool [[maybe_unused]] ok = packet.decrypt(&unused, ptr.get(), PacketSsl::get_buffer_size(msg_sz));
  if (!ok) {
