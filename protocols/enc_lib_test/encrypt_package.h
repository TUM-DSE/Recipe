...
#include <utility>
#if NO_CIPHER
#warning "We are going to calculate the hashes"
#include <city.h>
#endif
...
private:
  ...
  static void inc(std::byte *dst, std::byte *src, INT const a) noexcept {
    ...
    asm("add   %[a], %[low]\n"
        "adc   $0,   %[high]\n"
        : [low] "=rm"(dst_val->low), [high] "=rm"(dst_val->high)
        : [a] "erm"(a), "[low]"(src_val->low), "[high]"(src_val->high)
        : "cc");
  }
...
public:
  void get_packet_iv_aligned(std::byte *dst) const noexcept {
    ...
    { inc(raw, dst, 1); }
    while (!secure_replication::compare_exchange_128(iv, raw, dst))
      ;
  }
...
/**
 * Can de-/encrypt packet buffers the data structure of an encrypted buffer will
 * look like
 * +-------+-----------+-------+
 * |   IV  |    data   |  MAC  |
 * +-------+-----------+-------+
...
public:
  ...
  explicit PacketSsl(std::shared_ptr<KeyIV> key_iv)
      : key_iv(std::move(key_iv)) {}
  ...
  static PacketSsl create(CipherSsl const &cipher) {
    return PacketSsl(std::make_shared<KeyIV>(
        reinterpret_cast<std::byte const *>(cipher._key),
        reinterpret_cast<std::byte const *>(cipher._iv)));
  }

  // returns the minimum size the buffer for encryption must have
  [[gnu::pure, nodiscard]] static size_t constexpr get_buffer_size(
      size_t const msg_sz) noexcept {
#if NO_CIPHER
    return msg_sz;
#else
  ...
  [[gnu::pure, nodiscard]] static size_t constexpr get_message_size(
      size_t const buf_sz) noexcept {
#if NO_CIPHER
    return buf_sz;
#else
  ...
  [[gnu::pure, nodiscard]] static size_t constexpr adjust_size(
      size_t const msg_sz) noexcept(noexcept(get_buffer_size(msg_sz))) {
    return get_buffer_size(msg_sz);
  }

private:
  template <Func func>
  static bool cipher_final(EVP_CIPHER_CTX *pState, unsigned char *p_dst,
                           int *len, unsigned char *mac, bool verify) noexcept {
    // if constexpr (func == ENCRYPT) {
    if (!verify) {
      bool ret  = EVP_CipherFinal_ex(pState, p_dst, len) &&
             EVP_CIPHER_CTX_ctrl(pState, EVP_CTRL_GCM_GET_TAG, MacSize, mac);
#if 0
    std::cout << "MAC=";
      for (auto i = 0; i < MacSize; i++) {
	      std::cout << std::hex << std::setw(2) << static_cast<int>(mac[i]) << " ";
      }
      std::cout << "\n";
#endif
      return ret;
    ...
    // std::cout << "\nEDW\n";
	uint8_t calc_mac[MacSize];
      bool ret  = EVP_CipherFinal_ex(pState, NULL, len) &&
             EVP_CIPHER_CTX_ctrl(pState, EVP_CTRL_GCM_GET_TAG, MacSize, calc_mac);
#if 0
    std::cout << "CALC MAC=";
      for (auto i = 0; i < MacSize; i++) {
...
#endif
      if ((::memcmp(calc_mac, mac, MacSize) == 0) && (ret))
	      return true;
      else
...
public:
  ...
  bool cipher(DST *dst, SRC *src, size_t size) const noexcept {
    ...
    auto p_dst = reinterpret_cast<unsigned char *>(
        const_cast<typename std::remove_const<DST>::type *>(dst));
    auto p_src = reinterpret_cast<unsigned char *>(
        const_cast<typename std::remove_const<SRC>::type *>(src));
...
/*
...
#if 0
    std::cout << "IV=";
    for (auto i = 0Ull; i < IvSize; i++) {
	std::cout << std::hex << std::setw(2) << static_cast<int>(iv[i]) << " ";
    ...
    auto ret = pState &&
           EVP_CipherInit_ex(
               pState.get(), EVP_aes_128_gcm(), nullptr,
               reinterpret_cast<unsigned char const *>(key_iv->key), iv,
               ENCRYPT) &&
               /*func) &&*/
            // EVP_CipherUpdate(pState.get(), p_dst, &len, p_src, size) &&
           EVP_CipherUpdate(pState.get(), NULL, &len, p_src, size) &&
           cipher_final<func>(pState.get(), p_dst, &len, mac, (func == DECRYPT) );
    // std::cout << "len=" << len << " ret= " << ret << "\n";
    return ret;
  }

  // en-/decrypts message/buffer
  // equivalent to calling encrypt or decrypt however with the func parameter as
  // determinator for encryption or decryption
  template <class DST, class SRC>
  auto cipher(DST *dst, SRC *src, size_t src_size, Func func) const noexcept
      -> decltype(cipher<ENCRYPT>(dst, src, src_size)) {
    if (func == ENCRYPT) {
...
// #warning "CITYHASH COMMENTED out"
#if 1
    auto key_hash_ = CityHash64((char *)src, src_size);
    if (key_hash_ != h) {
      return false;
    }
#endif
    return true;
#else
#warning "GMAC ENCRYPTION is ON"
    // std::cout << __PRETTY_FUNCTION__ << "\n";
    // std::cout << "ENCRYPT " << src_size << "\n";
    return cipher<ENCRYPT>(dst, src, src_size);
#endif
  }
  ...
  auto decrypt(DST *dst, SRC *src, size_t src_size) const noexcept
      -> decltype(cipher<DECRYPT>(dst, src, src_size)) {
#if NO_CIPHER
#warning "NO ENCRYPTION"
    memcpy(dst, src, src_size);
// #warning "CITYHASH COMMENTED out"
    ...
    return true;
#else
#warning "ENCRYPTION is ON"
    // std::cout << "DECRYPT " << src_size << "\n";
    return cipher<DECRYPT>(dst, src, src_size);
#endif
  }
};
