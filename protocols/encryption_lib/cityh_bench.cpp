...
int main(int argc, char **argv) {
...
#endif
	std::cout << "msg_sz=" << msg_sz << " w/ cityhash\n";
	...
	std::unique_ptr<uint8_t[]> ptr =
		std::make_unique<uint8_t[]>(PacketSsl::get_buffer_size(sizeof(uint64_t)));

	auto start = get_time_in_ms();

	for (auto i = 0; i < 10e6;i++) {

		auto h1 = CityHash64(reinterpret_cast<char*>(aad.get()), msg_sz);
		packet.encrypt(ptr.get(), &h1, sizeof(h1));
		...
		std::unique_ptr<uint8_t[]> decrypted = std::make_unique<uint8_t[]>(PacketSsl::get_message_size(PacketSsl::get_buffer_size(sizeof(uint64_t))));
		bool [[maybe_unused]] ok = packet.decrypt(decrypted.get(), ptr.get(), PacketSsl::get_buffer_size(sizeof(h1)));
		if (!ok) {
			std::cout << "Decryption failed\n";
		}
		uint64_t h2;
		::memcpy(&h2, decrypted.get(), sizeof(h2));
		if (h2 != CityHash64(reinterpret_cast<char*>(aad.get()), msg_sz)) {
