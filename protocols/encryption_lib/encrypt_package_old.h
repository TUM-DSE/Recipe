...
private:
  ...
  static bool cipher_final(EVP_CIPHER_CTX *pState, unsigned char *p_dst,
                           int *len, unsigned char *mac) noexcept {
  ...
  template <Func func>
  static bool cipher_update(EVP_CIPHER_CTX *pState, unsigned char *p_dst,
                            int *len, unsigned char *p_src, Size_t &size) {
    if constexpr (func == ENCRYPT) {
      return EVP_CipherUpdate(pState, p_dst, len,
                              reinterpret_cast<unsigned char const *>(&size),
                              sizeof(size)) &&
             EVP_CipherUpdate(pState, p_dst + *len, len, p_src, size);
    }
    auto max_size = size;
    size = 0;
    return EVP_CipherUpdate(pState, reinterpret_cast<unsigned char *>(&size),
                            len, p_src, sizeof(size)) &&
           (size <= max_size) &&
...
public:
  ...
  template <class DST, class SRC>
  auto encrypt(DST *dst, Size_t dest_size, SRC *src,
               Size_t src_size) const noexcept
      -> decltype(cipher<ENCRYPT>(dst, src, src_size)) {
    return cipher<ENCRYPT>(dst, src, src_size);
  }

  template <class DST, class SRC>
  auto decrypt(DST *dst, Size_t dest_size, SRC *src,
               Size_t src_size) const noexcept
      -> decltype(cipher<ENCRYPT>(dst, src, src_size)) {
    return cipher<DECRYPT>(dst, src, src_size);
  }
