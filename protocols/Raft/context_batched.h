...
static [[nodiscard]] std::unique_ptr<uint8_t[]> _decode_req(char *data,
	...
	// fmt::print("[{}] enc_data={} < data_sz = {}\n", __PRETTY_FUNCTION__, enc_data, data_sz);
	//FIXME: refactoring with copies is bad.. std::move() next time
...
#if GMAC
 #warning "GCMAC is ON"
                 cipher->encrypt(tag_ptr->req_buf.buf, data.get(), sizeof(h) + sizeof(follower_cmt_idx));
                 ::memcpy(tag_ptr->req_buf.buf+PacketSsl::IvSize, data.get(), sizeof(h) + sizeof(follower_cmt_idx));
 #else
...
static void sent_new_commit_index(AppContext *context,
	...
	if ((context->ptr_state->uncommitted_reqs[idx]->recv_acks.size() >=
		...
		for (auto i = (idx); i <= hdr.op_id; i++) {
			auto &reqs = queued_reqs[i];
			if (reqs == nullptr) {
				// fmt::print("[{}] nullptr for i={}\n", __func__, i);
				break;
			}
			if (reqs->recv_acks.size() >= state::nb_followers) {

				for (int nb_r = 0; nb_r < reqs->nb_batched_reqs; nb_r++) {
					auto &key = reqs->keys[nb_r];
          				auto& value = reqs->values[nb_r];
          				context->store->put(ycsb::Trace_cmd::key_size, key.get(), value.get(), kValueSize);
					// context->store->put(ycsb::Trace_cmd::key_size, key.get());
					// print_key(key.get());
					...
					flag = true;
#if 0
					for (auto& owner: reqs->reqs_owners) {
						fmt::print("[{}] req_owner={}\n", __func__, owner);
					}
#endif
					// fmt::print("[{}]
					// reqs->reqs_owners.size()={}/reqs->nb_batched_reqs={}
					// hdr.op_id={}\n", __func__, reqs->reqs_owners.size(),
					// reqs->nb_batched_reqs, hdr.op_id);
#if FOLLOWER_WORKLOAD || RATELIMITER
					auto req_owner = reqs->reqs_owners[nb_r];
					if (req_owner != 0) {
						context->ptr_state->follower_cmt_idx[req_owner]++;
					}
#if 0
					else 
						rate.release(1);
...
static void cont_func_cmt(void *ctx, void *tag) {
	...
	context->ptr_state->commit_acks[hdr.node_id]->fetch_add(hdr.batch_sz);
#if 0
	if (hdr.latest_cmt > context->ptr_state->commit_acks[hdr.node_id]->load())
		context->ptr_state->commit_acks[hdr.node_id]->store(hdr.latest_cmt);
#endif

	if (context->ptr_state->commit_acks[hdr.node_id]->load() % 20000 == 0)
		fmt::print("[{}] commit_acks for hdr.node_id={}/RID={} is {} w/ latest cmt_idx={}\n", __func__,
				hdr.node_id, context->RID,
				context->ptr_state->commit_acks[hdr.node_id]->load(), hdr.latest_cmt);
