...
int main(int args, char *argv[]) {
  ...
  std::string server_uri = follower_address_1 + ":" + std::to_string(kUDPPort);
  int cur_node_id = 1;
...
//  avocado::KV_store store(std::move(crypt));
  ...
  threads.push_back(std::thread(raft_follower_worker, &nexus, std::move(ptr_id),
  ...
  for (size_t i = 0; i < kNumThreads; i++) {
    ...
    threads.push_back(std::thread(raft_follower_thread_func, &nexus,
                                  std::move(ptr_id), arguments[i].get(),
                                  ptr_state, cur_node_id, &store));
  }
