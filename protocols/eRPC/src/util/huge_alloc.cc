...
namespace erpc {
	...
	HugeAlloc::~HugeAlloc() {
		...
		fprintf(stdout, "[%s], # of mmap() calls %d\n", __PRETTY_FUNCTION__, nb_mmap_calls);
		// print_stats();
	}
	...
	void HugeAlloc::print_stats() {
		...
		auto total_mem_allocated_b = 0;
		auto total_mem_allocated_kb = 0;
		auto total_mem_allocated_mb = 0;
		...
		for (size_t i = 0; i < kNumClasses; i++) {
			...
			if (class_size < KB(1)) {
				...
				total_mem_allocated_b += (class_size)*freelist[i].size();
			} else if (class_size < MB(1)) {
				fprintf(stderr, "\t%zu KB: %zu Buffers\n", class_size / KB(1),
						freelist[i].size());
				total_mem_allocated_kb += (class_size/KB(1))*freelist[i].size();
			} else {
				fprintf(stderr, "\t%zu MB: %zu Buffers\n", class_size / MB(1),
						freelist[i].size());
				total_mem_allocated_mb += (class_size/MB(1))*freelist[i].size();
			}
		}
		fprintf(stdout, "Total mem allocated: \t%zu B,\t%zu KB,\t%zu MB\n", total_mem_allocated_b, total_mem_allocated_kb, total_mem_allocated_mb);
		if (total_mem_allocated_b > KB(1)) {
			auto nb_kb = total_mem_allocated_b / KB(1);
			total_mem_allocated_kb += nb_kb;
			total_mem_allocated_b = total_mem_allocated_b % KB(1);
		}
		if (total_mem_allocated_kb*KB(1) > MB(1)) {
			auto nb_mb = (total_mem_allocated_kb*KB(1)) / MB(1);
			total_mem_allocated_mb += nb_mb;
			total_mem_allocated_kb = (total_mem_allocated_kb*KB(1)) % MB(1);
		}
		fprintf(stdout, "[Reduced] Total mem allocated (allocated_mem: \t%zu MB, freed_mem: \t%zu MB user_alloc_mem: \t%zu): \t%zu B,\t%zu KB,\t%zu MB\n", allocated_mem.load()/MB(1), freed_mem.load()/MB(1), user_allocated_mem.load()/MB(1), total_mem_allocated_b, total_mem_allocated_kb, total_mem_allocated_mb);
	}
...
#warning SHM_HUGETLB causes `Invalid Argument` error when invoking mbind() 
		...
		if (mem == -1) {
			rt_assert("opening /dev/zero problem");
		}
		auto shm_buf = static_cast<uint8_t *>(scone_kernel_mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_PRIVATE, mem, 0));
		nb_mmap_calls++;
		fprintf(stdout, "[%s], # of mmap() calls %d\n", __PRETTY_FUNCTION__, nb_mmap_calls);
		if (shm_buf == nullptr)
			rt_assert("mmap failed");
#endif
#if 1
		allocated_mem.fetch_add(size);
//		std::cout << __func__ << " " << size / MB(1) << " MB " << (allocated_mem.load()/MB(1)) << " " << GB(2) << "\n";
#endif
		...
		// rt_assert(ret == 0, "eRPC HugeAlloc: mbind() failed. Key " + std::to_string(shm_key) + " size : " + std::to_string(size) + " " + std::strerror(errno));
