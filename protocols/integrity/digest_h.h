...
#include <memory>
#if SHA256_DIGEST
constexpr int kSHA256_SZ=EVP_MAX_MD_SIZE/2;

static std::tuple<size_t, std::unique_ptr<uint8_t[]>> get_sha256(const char* data,
		size_t data_sz) {
	...
	if (md_len != kSHA256_SZ) {
		std::cout << "md_len=" << md_len << "\n";
		exit(128);
	}
	EVP_MD_CTX_free(mdctx);
	return std::make_tuple(md_len, std::move(md_value));
}
...
static std::tuple<size_t, std::unique_ptr<char[]>> construct(PacketSsl* cipher, char* metadata, size_t meta_sz, char* data, size_t data_sz) {
	auto [h_sz, h] = get_sha256(data, data_sz);
	std::unique_ptr<char[]> ptr = std::make_unique<char[]>(meta_sz + h_sz);
	::memcpy(ptr.get(), metadata, meta_sz);
	::memcpy(ptr.get() + meta_sz, h.get(), h_sz);
	std::unique_ptr<char[]> enc = std::make_unique<char[]>(PacketSsl::get_buffer_size(meta_sz + h_sz));
	bool ok = cipher->encrypt(enc.get(), ptr.get(), (meta_sz+h_sz));
	// std::cout <<__PRETTY_FUNCTION__ << " h_sz=" << h_sz <<" (meta_sz+h_sz)=" << (meta_sz+h_sz) << "\n";
	if (!ok)
		std::cout << __PRETTY_FUNCTION__ <<  " error sha256\n";

	return std::make_tuple(PacketSsl::get_buffer_size(meta_sz + h_sz), std::move(enc));
}
...
static std::unique_ptr<uint8_t[]>  destruct(PacketSsl* cipher, char* enc_data, size_t enc_sz) {
	...
	//std::cout << __PRETTY_FUNCTION__ << " " << enc_sz << "\n";
...
static std::tuple<size_t, std::unique_ptr<char[]>> _construct(PacketSsl* cipher, char* metadata, size_t meta_sz, char* data, size_t data_sz) {
...
static std::unique_ptr<uint8_t[]>  _destruct(PacketSsl* cipher, char* enc_data, size_t enc_sz) {
	size_t decrypted_data_sz = PacketSsl::get_message_size(enc_sz);
	std::unique_ptr<uint8_t[]> decrypted_data =
		std::make_unique<uint8_t[]>(decrypted_data_sz);
	bool [[maybe_unused]] success =
